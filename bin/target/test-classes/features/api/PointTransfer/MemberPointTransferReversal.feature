...
Scenario: Partial point transfer reversal is not allowed
	...
	And source member should not be able to do partial point transfe reversal with status code 422
	And Response message template should show message "Insufficient deposit to perform the operation, current balance: {balance}"
