...
Scenario: Enroll source and target member and add cards to source member 
	...
	And Source member perform create legal entity
	And Source member perform create card with legal entity relation 
    ...
    And Legal entity for source member should be successful with statuscode 200 
...
Scenario: To verify the source member balance after transactions 
	...
	And Source member perform generate a voucher token
	...
	And Earn and redeem balance of the target member should be displayed with status code<status_code> 
...
Scenario: To verify that the source member can do earn with source card after merging 
	...
	#Then Earn transaction should not be successful for the source member with status code 200
...
Scenario: To verify that the target member redeemable balance will be sum of the redeemable balance of source and target member balance after merging 
	...
	#scanrion : Source member could not see his deposit statement	
...
Scenario: To verify that the source member points expiry should be same after merging
...
@NewVersion		
Scenario: To verify that the target member contain the source member transaction after merging 
...
@NewVersion		
Scenario: To verify that the target member do point transfer from one member to another 
	Given Target member wants to transfer points to another member 
	When member performs enrollment of another member
	And Target member perform points transfer
	Then another member is created with statuscode 200
	And point transfer from target to another member is successful with status code 200
	#And Transaction history of the target member should include all the transaction detail of source member with status code 200 
	#And Transaction history of the target member should contain the fields "transferredTo" and "transferReason" after merging 
	#And Target member total transactions history count should be equal to the sum of the source and target member after merge 
		
	#ForVouchers
@NewVersion	
Scenario: To verify that the source vouchers are transferred to target member after merge 
...
Scenario: To verify that the target member can convert the voucher issue by the source member
	...
	Then Target member should be able to convert the voucher successful with status code 200
...
Scenario: To verify that the token generated for a source voucher can be issued to target member after merge
	...
	Then Target member should be able to issue the voucher successfully with status code 200 		
...
#miscellaneous
Scenario: To verify that the source member's membership status is canceled after merging
	Given Source member wants to see his membership status
	When Source member perform membership status 
	Then Source member should be able to see the membership status as canceled with status code 200
